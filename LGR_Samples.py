...
def input_data(sample_data, LGR_data):
    ...
    for line in f:
        ...
        for k in range(len(x)):
            x[k] = x[k].strip(' \t\n\r')
        time_match = re.search(time_regex, x[LGR_time_index])
        if time_match:
            time = float(time_match[1])*3600 + float(time_match[2])*60 + float(time_match[3])
            LGR.append([time, float(x[LGR_CH4_index]), float(x[LGR_CO2_index])])

    # extracting sample information from sample file
...
def process_samples(samples, LGR):
...
def on_press(event, i, samples, line_L, line_R, LGR, fig, ax, cid):
    sys.stdout.flush()
    ...
    if event.key == 'right' or event.key == 'space':
        if i == len(samples) - 1:
            ...
            draw_plot(i + 1, samples, LGR, fig, ax, cid)
...
def draw_plot(i, samples, LGR, fig, ax, cid):
    ...
    if len(samples[i].times) == 0:
        ...
        for j in range(len(LGR)):
            if float(samples[i].peak_start_time) - float(LGR[j][0]) < 0 and LGR_start == 0:
                LGR_start = j
            if float(samples[i].peak_end_time) - float(LGR[j][0]) < 0 and LGR_end == 0:
                LGR_end = j
        for k in range(math.floor(LGR_start), math.ceil(LGR_end)):
            samples[i].times.append(float(LGR[k][0]))
            samples[i].concentrations_CH4.append(float(LGR[k][1]))
            samples[i].concentrations_CO2.append(float(LGR[k][2]))
    ...
    cid = fig.canvas.mpl_connect('key_press_event', lambda event: on_press(event, i, samples, line_L, line_R, LGR, fig, ax, cid))   # connect key press event  
    ax.grid(True)
    fig.canvas.draw()


# obtains peak bounds from user interactive plots
def obtain_peaks(samples, LGR):
    i = 0
    fig, ax = plt.subplots()
    ...
    plt.grid(True)
    plt.ion()
    draw_plot(i, samples, LGR, fig, ax, cid)
...
def standardize(samples, LGR):
    for j in range(len(samples)):
        LGR_start = 0           # sample start index within sample time and concentration sets (not FMA data indices)
        LGR_end = 0             # sample end index within sample time and concentration sets (not FMA data incides)
        LGR_cut_L = 0           # sample start index within FMA data set
        LGR_cut_R = 0           # sample end index within FMA data set
        ...
        samples[j].times = samples[j].times[LGR_start: LGR_end + 1]
        samples[j].concentrations_CH4 = samples[j].concentrations_CH4[LGR_start: LGR_end + 1]
        samples[j].concentrations_CO2 = samples[j].concentrations_CO2[LGR_start: LGR_end + 1]

        # cuts out sample from overall FMA data (used for baseline sampling in generating linear model)
        for k in range(len(LGR)):
            if float(LGR[k][0]) == samples[j].times[0]:
                LGR_cut_L = k
            if float(LGR[k][0]) == samples[j].times[-1]:
                LGR_cut_R = k
        LGR = LGR[0: LGR_cut_L] + LGR[LGR_cut_R + 1: -1]
...
def linear_model(samples, LGR):
    ...
    for i in range(numStandards):
        ...
        Y_CO2.append(float(LGR[random.randint(0, len(LGR))][2]))
        X_CO2.append(0)
...
def LGR_Samples():
    ...
    layout = [[sg.Text('LGR sample Data Processing Tool', font='Any 36', background_color='#A14500')],
        [sg.Text("", background_color='#A14500')],
        [sg.Text('Sample data file: (.csv, .txt)', size=(21, 1), background_color='#A14500'), sg.Input(key='-SAMPLES-'), sg.FileBrowse()],
        [sg.Text('LGR data file: (.csv, .txt, .data)', size=(21, 1), background_color='#A14500'), sg.Input(key='-LGR-'), sg.FileBrowse()],
        [sg.Text("", background_color='#A14500')],
        [sg.Submit(), sg.Cancel()]]
    ...
    # Create the window
    window = sg.Window("LGR samples", layout, margins=(80, 50), background_color='#A14500')
    cancelled = False
    ...
    if cancelled == False:
        ...
        try:
            ...
            samples, LGR = input_data(sample_data, LGR_data)

            print("Processing data")
            process_samples(samples, LGR)

            print("Obtaining peak bounds")
            obtain_peaks(samples, LGR)

            print("Trimming data")
            standardize(samples, LGR)

            print("Calculating peak areas")
            peak_areas(samples)

            print("Generating linear model")
            (m_CH4, b_CH4, R2_CH4, m_CO2, b_CO2, R2_CO2) = linear_model(samples, LGR)
